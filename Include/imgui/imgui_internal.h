...
struct IMGUI_API ImRect
{
    ...
    [[nodiscard]] bool        ContainsWithPad(const ImVec2& p, const ImVec2& pad) const { return p.x >= Min.x - pad.x && p.y >= Min.y - pad.y && p.x < Max.x + pad.x && p.y < Max.y + pad.y; }
...
struct IMGUI_API ImGuiInputTextState
{
    ...
    [[nodiscard]] int         GetRedoAvailCount() const   { return IMSTB_TEXTEDIT_UNDOSTATECOUNT - Stb.undostate.redo_point; }
